...
class CalDAVToolKit(BaseToolKit[CalDAVSettings]):
    ...
    async def get_calendars(self) -> list[Calendar]:
        ...
        return [
            await self._convert_calendar_to_dto(calendar=dav_calendar)
    ...
    async def create_calendar(self, name: str) -> Calendar:
        ...
        return await self._convert_calendar_to_dto(calendar=dav_calendar)
    ...
    async def get_events(
        ...
        if calendar_url is not None:
            dav_calendars = [AsyncCalendar(client=self._client, url=calendar_url)]
        else:
            principal = await self.get_principal()
            dav_calendars = await principal.get_calendars()
    ...
    async def create_event(
        ...
        Args:
            ...
            start: Event start datetime (ISO 8601 format with timezone)
            end: Event end datetime (ISO 8601 format with timezone)
        ...
        event_data = (
            ...
            "BEGIN:VEVENT\n"
            f"SUMMARY:{summary}\n"
            f"DTSTART:{datetime.fromisoformat(start).strftime(self.DATETIME_FORMAT)}\n"
            f"DTEND:{datetime.fromisoformat(end).strftime(self.DATETIME_FORMAT)}\n"
        )
        if description:
    ...
    async def update_event(
        ...
        Args:
            ...
            start: New event start datetime (datetime, date, or ISO 8601 string) (optional)
            end: New event end datetime (datetime, date, or ISO 8601 string) (optional)
        ...
        for component in event_instance.subcomponents:
            ...
            if start is not None:
                component["DTSTART"] = datetime.fromisoformat(start).strftime(self.DATETIME_FORMAT)
            if end is not None:
                component["DTEND"] = datetime.fromisoformat(end).strftime(self.DATETIME_FORMAT)

        dav_event.data = str(event_instance)
        dav_event.save()
        return await self._convert_event_to_dto(dav_event)
