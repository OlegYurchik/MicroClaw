...
class EmailFolder(BaseModel):
    ...
    flags: list[str] = Field(
        default_factory=list, description="Folder flags (e.g., \\HasNoChildren)"
    )
...
class EmailAttachment(BaseModel):
    ...
    content_id: str | None = Field(
        default=None, description="Content-ID for inline attachments"
    )
...
class EmailMessage(BaseModel):
    ...
    bcc: list[str] = Field(
        default_factory=list, description="List of BCC email addresses"
    )
    date: datetime = Field(description="Date the email was sent")
    folder: str = Field(description="Folder where the message is stored")
    flags: list[str] = Field(
        default_factory=list, description="Message flags (e.g., \\Seen, \\Flagged)"
    )
